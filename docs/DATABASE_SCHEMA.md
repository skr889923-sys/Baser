...
يعتمد تطبيق **دلّني (Dallni)** على قاعدة بيانات **Supabase (PostgreSQL)** لإدارة المباني، المسارات الملاحية، رموز الاستجابة السريعة، البلاغات الميدانية وطلبات الاستغاثة.
...
- `code` (text, unique): الرمز النصي المشفر المدعوم في التطبيق (مثال: `dallni://point/{uuid}`).
